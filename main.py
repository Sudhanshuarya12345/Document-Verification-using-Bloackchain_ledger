...
def issue_command(args: argparse.Namespace) -> int:
    ...
    file_hash = sha256_file(file_path)
    duplicate_record = find_document_record(chain, file_hash, None)
    if duplicate_record is not None:
        duplicate_payload = duplicate_record.payload
        print("Cannot issue document. Duplicate file already exists on chain.")
        print(f"Existing block index : {duplicate_record.index}")
        print(f"Existing Document ID : {duplicate_payload.get('document_id')}")
